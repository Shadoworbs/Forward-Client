...
async def Kanger(c: Client, m: Message):
    ...
    for i in range(len(Config.FORWARD_TO_CHAT_ID)):
        ...
        try:
            ForwardToChat = await c.get_chat_member(
                chat_id=Config.FORWARD_TO_CHAT_ID[i], user_id=(await c.get_me()).id
            )
            if not ForwardToChat.permissions.can_send_messages:
                await m.edit(
                    text=f"Sorry, you don't have permission to send messages in {ForwardToChat.title} !"
                )
                return 400
            ...
            await m.edit(
                text=f"Sorry, can't get **Forward To Chat**!\n\n**Error:** `{err}`"
            )
            return 400
    await asyncio.sleep(2)
    await m.edit(text="Trying to Forward Now ...")
    async for message in await c.get_chat_history(chat_id=int(Config.FORWARD_FROM_CHAT_ID[0])):
    ...
    await m.edit(
        text="Channel Files Successfully Kanged!\n\n©️ A Forwarder Userbot by @shadoworbs"
    )
