...
async def main(client: Client, message: Message):
    ...
    if (message.text == "/start") and message.from_user.is_self:
        ...
        if Config.HEROKU_APP is None:
            ...
            time.sleep(30)
    elif (message.text == "/forward") and message.from_user.is_self and RUN["isRunning"]:
        if len(Config.FORWARD_FROM_CHAT_ID) > 1:
            await message.edit(
                text="Sorry Sir,\n"
                     "I can forward only one Chat! But you put multiple Chat IDs in `FORWARD_FROM_CHAT_ID` Config!",
                disable_web_page_preview=True
            )
            return
        await message.edit(
        ...
        try_to_forward = await Kanger(c=User, m=message)
        if try_to_forward == 400:
            return
